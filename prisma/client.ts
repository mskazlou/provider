...
/**
 * to Connect our applications to a database, we ofter use an Object-relational
 ...
 *  These are entities that represent our application domain, such as User, Order,
 *  Customer, etc. Each model has one or more fields (or properties).
 ...
 *   or update database tables to match our models. These instructions are in SOL
 ...
 *  create a new migration. It exposes properties that represent our models (e.g.
 *  user) .
 * At `./prisma/client.ts` copy & paste this code
 * ```ts
 * import (PrismaClient) from '@prisma/client'
 * const globalForPrisma = global as unknown as f
 * prisma: PrismaClient | undefined
